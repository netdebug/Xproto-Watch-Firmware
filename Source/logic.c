...
void Sniff(void) {
    ...
    while((Temp.LOGIC.indrx==0) && (Temp.LOGIC.indtx==0)) {
        WDR();
        if(M.CHDdecode!=i2c) {
            Temp.LOGIC.indrx=BUFFER_SERIAL-DMA.CH0.TRFCNT;
            Temp.LOGIC.indtx=BUFFER_SERIAL-DMA.CH1.TRFCNT;            
        }            
        if(M.CHDdecode==spi) {
    ...
    do {
        ...
        if(M.CHDdecode==i2c) {
            if(testbit(Trigger, round)) {
                if(Temp.LOGIC.indrx>=DATA_IN_PAGE_I2C) {
                    i=Temp.LOGIC.indrx-DATA_IN_PAGE_I2C;
                }
                else {
                    i=(BUFFER_I2C-DATA_IN_PAGE_I2C)+Temp.LOGIC.indrx;
                }
